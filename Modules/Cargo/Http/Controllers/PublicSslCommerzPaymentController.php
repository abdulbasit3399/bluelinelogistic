...
class PublicSslCommerzPaymentController
{
    ...
    public function index($shipment)
    {
            ...
            //     if(Session::get('payment_type') == 'cart_payment'){
                    $post_data = array();
                    $post_data['total_amount'] = $shipment->tax + $shipment->shipping_cost + $shipment->insurance; # You cant not pay less than 10
                    $post_data['currency'] = \Modules\Currency\Entities\Currency::where('default',1)->first()->code;
                    $post_data['tran_id'] = substr(md5($shipment->id), 0, 10); // tran_id must be unique

                    $post_data['value_a'] = $post_data['tran_id'];
                    $post_data['value_b'] = $shipment->id;
                    $post_data['value_c'] = $shipment->payment_method_id;
                    ...
                    $post_data['cus_name'] = $shipment->client->name;
                    $post_data['cus_add1'] = $shipment->from_address->address;
                    $post_data['cus_city'] = $shipment->to_state->name;
                    $post_data['cus_postcode'] = "1234";
                    $post_data['cus_country'] = $shipment->to_country->name;
                    $post_data['cus_phone'] = $shipment->client->responsible_mobile;
                    $post_data['cus_email'] = $shipment->client->email;
                // }
    ...
    public function success(Request $request)
    {
        ...
        if(isset($request->value_c)){
            // if($request->value_c == 'cart_payment'){
                ...
                return $checkoutController->checkout_done( null, $payment, $shipment_id);
            // }
    ...
    public function fail(Request $request)
    {
        ...
        $shipment = Shipment::find($request->value_b);
        flash('Payment Failed')->success();
    	return redirect()->route('shipments.show', ['shipment'=>$shipment->id]);
    }
