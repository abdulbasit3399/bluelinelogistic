...
<!-- begin: Btn Edit Row -->
@if(auth()->user()->can('edit-packages') || $user_role == $admin )
    <a
        href="{{ fr_route('packages.edit', $model->id) }}"
...
@if(auth()->user()->can('delete-packages') || $user_role == $admin )
    <button
        type="button"
        data-action="{{ fr_route('packages.destroy', $model->id) }}"
