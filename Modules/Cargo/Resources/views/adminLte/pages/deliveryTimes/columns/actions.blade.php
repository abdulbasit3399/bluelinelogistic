...
<!-- begin: Btn Edit Row -->
@if(auth()->user()->can('edit-delivery-time') || $user_role == $admin )
    <a
        href="{{ fr_route('deliveryTime.edit', $model->id) }}"
...
@if(auth()->user()->can('delete-delivery-time') || $user_role == $admin )
    <button
        type="button"
        data-action="{{ fr_route('deliveryTime.destroy', $model->id) }}"
