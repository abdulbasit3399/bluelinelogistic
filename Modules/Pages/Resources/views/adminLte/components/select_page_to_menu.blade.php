        ...
        <div class="accordion-section-content card-body">
            <div class="inside">
                <div class="customlinkdiv">
                    ...
                    <div class="mb-6">
                        <!--begin::Label-->
                        <label class="mb-3 fw-bold fs-6" for="select_static_pages">@lang('pages::view.select_pages')</label>
                        <!--end::Label-->
    
                        <!--begin::Input group-->
                        <div class="input-content">
                            <select
                                class="form-control form_select_static"
                                name="pages[]"
                                multiple
                                id="select_static_pages"
                                data-placeholder="@lang('pages::view.pages_table.choose_page')"
                            >
                            </select>
                    ...
                    <div class="button-controls">
                        <button type="button" onclick="addCustomMenuMulti('static')" class="btn btn-sm float-end btn-primary">
                            @lang('menu::view.add_menu_item')
                            <span class="spinner fas fa-circle-notch fa-spin" id="loading_page"></span>
                        </button>
                    </div>
...
<script>
        ...
        // select pages
		var selectStaticPages = $('.form_select_static');
        selectStaticPages.select2({
            closeOnSelect: false,
            ajax: {
                url: "{{ fr_route('static_pages.search') }}",
                dataType: 'json',
                delay: 500,
                data: function (params) {
                    return { search: params.term };
                ...
                processResults: function (data) {
                    if (data && data.pages) {
                        return {
                            results: data.pages.map(function(page) {
                                return {id: page.id, text: page.title, data: page.title}
                            })
                        };
                    }
                },
